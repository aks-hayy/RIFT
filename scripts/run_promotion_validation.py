"""Run a focused real-device tuning promotion validation.

This intentionally starts from a valid conservative deployment (one CPU
thread) so the tuning coordinator has measurable headroom to explore. The
model artifact and weight quantization remain locked. The report is copied to
the runtime evidence directory and the temporary service is always torn down.
"""
...
def main() -> int:
    ...
    output = ROOT / ".rift-runtime" / "reports" / "real-tuning-validation-final" / "raw" / "3b-llama-cpp-speed-promoted.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    # Use the already-qualified local llama.cpp binary; no install or network
    # permission is needed for this validation run.
    server = ROOT / ".rift-runtime" / "backends" / "llama.cpp" / "llama-server.exe"
    if not server.is_file():
        raise SystemExit(f"llama-server not found: {server}")
    os.environ["LLAMA_CPP_SERVER"] = str(server.resolve())
    temp_root = Path(tempfile.mkdtemp(prefix="rift-promote-3b-speed-"))
    service_name = "promote3bspeed"
    orch = None
    started = time.time()
    try:
        ...
        service["model"].update({
            "source": "local", "id": model_path, "selected_file": model_path,
            "local_path": model_path, "format": "gguf", "artifact": {},
            "quantization": "Q4_K_M",
        })
        service["serving"].update({
            "port": 19430, "context_length": 4096, "concurrency": 1,
            "tuning": {
                "gpu_layers": 999, "batch": 128, "ubatch": 128,
                "threads": 1, "threads_batch": 1, "parallel": 1,
                "flash_attn": "off", "ngram_speculation": False,
            },
        })
        ...
        if not deployed.get("applied"):
            raise RuntimeError(f"baseline deployment failed: {deployed}")
        observation = {}
        for _ in range(180):
        ...
        report["validation_elapsed_seconds"] = time.time() - started
        print({
            ...
            "final_improvement_interval": report.get("final_improvement_interval"),
        })
