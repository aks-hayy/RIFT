...
import { i as cn, n as RiftUnavailable, o as Terminal, r as rift, s as createLucideIcon } from "./unavailable-z8_2XgY7.js";
...
var keys = {
	...
	deploymentRecords: ["rift", "deployment-records"],
	service: (id) => [
		"rift",
		"service",
		id
	...
	benchmarkProfiles: ["rift", "benchmark-profiles"],
	benchmarkRuns: (id) => [
		"rift",
	...
	settings: ["rift", "settings"],
	evaluations: (id) => [
		"rift",
		"evaluations",
		id
	],
	operations: ["rift", "operations"],
	tuningRuns: (service, profile) => [
		"rift",
		"tuning",
		"runs",
		service ?? "all",
	...
	telemetryLatest: (service) => [
		"rift",
		"telemetry",
		"latest",
		service ?? "all"
	],
	telemetryReports: (service) => [
		"rift",
		"telemetry",
		"reports",
		service ?? "all"
	],
	telemetryAccounting: (service) => [
		"rift",
		"telemetry",
		"accounting",
		service
...
function useTelemetryLatest(service) {
	return shape(useQuery({
...
function useResourceReports(service) {
	return shape(useQuery({
...
function useServiceTelemetryAccounting(service) {
	return shape(useQuery({
		queryKey: service ? keys.telemetryAccounting(service) : [
			"rift",
			"telemetry",
			"accounting",
			"none"
...
function useBenchmarkSuiteRuns(serviceId) {
	return shape(useQuery({
		queryKey: serviceId ? keys.benchmarkRuns(serviceId) : [
			"rift",
			"benchmark-suite-runs",
			"none"
		],
...
function useEvaluations(serviceId) {
	return shape(useQuery({
		queryKey: serviceId ? keys.evaluations(serviceId) : [
			"rift",
			"evaluations",
			"none"
		],
...
function useTuningRuns(options = {}) {
	return shape(useQuery({
...
function useTuningCapabilities(service) {
	return shape(useQuery({
		queryKey: [
			"rift",
			"tuning",
...
function useTuningRun(id) {
	return shape(useQuery({
		...
		refetchInterval: (query) => {
			...
			return status === "RUNNING" || status === "QUEUED" ? 1e3 : false;
		},
		retry: false
	}));
}
...
//#endregion
export { useServiceTelemetryAccounting as A, Activity as B, useNodes as C, useResourceReports as D, useReports as E, useTuningCapabilities as F, useTuningRuns as I, SlidersHorizontal as L, useSettings as M, useTelemetryLatest as N, useRevisions as O, useTimeline as P, Settings2 as R, useNode as S, useRecommendations as T, useLogs as _, StatDot as a, useMeshServices as b, useBackends as c, useBenchmarks as d, useDeploymentRecords as f, useLatestPlan as g, useIncidents as h, SourceBadge as i, useServices as j, useService as k, useBenchmarkProfiles as l, useHealth as m, PageHeader as n, AppShell as o, useEvaluations as p, Panel as r, useActiveTuningRun as s, KV as t, useBenchmarkSuiteRuns as u, useMeshNodes as v, useOperations as w, useMeshTopology as x, useMeshServiceGroups as y, Layers as z };
