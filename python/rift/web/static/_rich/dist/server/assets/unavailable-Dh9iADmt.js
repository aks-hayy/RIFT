...
function planRequest(recommendationRunId, selector) {
...
function applyRequest(configPath, permissions) {
...
function mapRecommendation(value, index, runId) {
	...
	const estimatedBytes = numeric(raw.selected_download_bytes, numeric(raw.estimated_download_bytes));
...
async function recommendDetailed(input) {
	...
	const payload = await req("POST", "/recommend", {
...
var rift = {
	...
	getPlan: async (_id, signal) => currentPlan(signal),
	applyPlan: async (id, planHash, options) => {
		const payload = await req("POST", "/apply", applyRequest(options.configPath, options), void 0, 1800 * 1e3);
		if (payload.applied === false) throw new RiftApiError(409, "/apply", payload);
		return {
			planId: id,
			planHash,
			phase: "succeeded",
	...
	planYaml: async () => {
		const generated = await req("GET", "/generated-config");
		return JSON.stringify(generated, null, 2);
	},
	timeline: (signal) => req("GET", "/timeline", void 0, signal),
	logs: (signal) => req("GET", "/logs", void 0, signal),
	backends: (signal) => req("GET", "/backends", void 0, signal),
	reports: (signal) => req("GET", "/reports", void 0, signal),
	currentPlan,
	benchmarkService: async (service, prompt = "Explain what RIFT does in one sentence.", maxTokens = 32) => req("POST", "/benchmark", {
		service,
		prompt,
		max_tokens: maxTokens
	}),
	benchmarkSuite: async (service, warmups = 1, repetitions = 3) => req("POST", "/benchmark-suite", {
		service,
		warmups,
		repetitions
	}),
	tuneService: async (service, options = {}) => req("POST", "/tune", {
		service,
		live: options.live ?? false,
		allow_restart: options.allowRestart ?? false,
		candidate_limit: options.candidateLimit ?? 4
	}),
	recoverService: async (service) => req("POST", "/recover", {
		service,
		allow_launch: true,
		force: true
	}),
...
//#endregion
export { TriangleAlert as a, createLucideIcon as c, cn as i, RiftUnavailable as n, Terminal as o, rift as r, ExternalLink as s, Unavailable as t };
