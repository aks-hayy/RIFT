...
function mapMeshService(value) {
	...
	return {
		...
		policyHash: text(raw.policy_hash, text(raw.policyHash)) || void 0
	};
}
function mapMeshServiceGroup(value) {
	const raw = object(value);
	return {
		groupId: text(raw.group_id, text(raw.groupId)),
		serviceIds: list(raw.service_ids).map((item) => text(item)).filter(Boolean),
		defaultService: text(raw.default_service, text(raw.defaultService)) || void 0,
		gatewayPath: text(raw.gateway_path, text(raw.gatewayPath)) || void 0
	};
}
...
function mapBenchmarkProfile(value) {
	const raw = object(value);
	return {
		id: text(raw.id),
...
function mapBenchmarkSuiteRun(value) {
	const raw = object(value);
	...
	return {
		...
		durationSeconds: raw.duration_seconds == null ? void 0 : numeric(raw.duration_seconds),
		profileResults: Object.fromEntries(Object.entries(resultMap).map(([key, item]) => {
		...
		artifactManifest: text(raw.artifact_manifest) || void 0,
		planFile: text(raw.plan_file) || void 0
	};
}
async function benchmarkProfiles(signal) {
	return list((await req("GET", "/v2/benchmark-profiles", void 0, signal)).profiles).map(mapBenchmarkProfile);
}
async function listBenchmarkSuiteRuns(service, signal) {
	return list((await req("GET", `/v2/benchmarks${service ? `?service=${encodeURIComponent(service)}` : ""}`, void 0, signal)).runs).map(mapBenchmarkSuiteRun);
}
async function getBenchmarkSuiteRun(runId, signal) {
	return mapBenchmarkSuiteRun(await req("GET", `/v2/benchmarks/${encodeURIComponent(runId)}`, void 0, signal));
}
...
function mapRecommendation(value, index, runId) {
	...
	const artifactId = text(artifactSelection.artifact_id, text(selectedArtifact.artifact_id, text(raw.artifact_id, text(raw.selected_artifact_id, repo))));
...
var rift = {
	...
	listMeshServices: async (signal) => {
		return list((await req("GET", "/v2/mesh/services", void 0, signal)).services).map(mapMeshService);
	},
	listMeshServiceGroups: async (signal) => {
		return list((await req("GET", "/v2/mesh/service-groups", void 0, signal)).groups).map(mapMeshServiceGroup);
	},
	...
	tuningCapabilities: async (service, signal) => req("GET", `/v2/tuning/capabilities?service=${encodeURIComponent(service)}`, void 0, signal),
	...
	benchmarkPlan: async (service, profiles, research) => req("POST", "/v2/benchmark-plans", {
		service,
		profiles,
		...research ? { research } : {}
	}),
	benchmarkProfilesRun: async (service, profiles, options = {}) => mapBenchmarkSuiteRun(await resolveOperation(await req("POST", "/v2/benchmarks", {
		service,
		profiles,
