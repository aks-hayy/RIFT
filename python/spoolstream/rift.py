"""RIFT product-layer API built on the native SpoolStream backend."""
...
class RiftProductInfo:
    ...
    backend: str = "SpoolStream native backend"


class RiftEngine:
    """Hardware-aware deployment planner facade over the native runtime.

    R0 intentionally keeps this class thin. The existing C++/CUDA backend remains
    exposed through ``native`` while RIFT grows the planner, report, and serving
    layers above it.
    """
        ...
        *,
        ...
        if not candidates:
            add_candidate(
                ...
                ["Use rift recommend to select a better-fit model for this PC."],
            )
    ...
    def recommend_models(
        ...
        *,
        ...
        if simulated_hardware is not None:
            ...
            download_target = Path(download_root or output_dir or (Path.cwd() / ".rift" / "models"))
        ...
        if persist_run:
            try:
                store = RecommendationStore(
                    Path(run_store_root) if run_store_root else Path.cwd() / ".rift"
                )
    ...
    def run(
        ...
        *,
        ...
        if save_report:
            target = Path(report_path) if report_path else Path.cwd() / ".rift" / "latest.riftreport.json"
            target.parent.mkdir(parents=True, exist_ok=True)
    ...
    def load_report(self, run: str = "latest") -> dict[str, Any]:
        path = Path.cwd() / ".rift" / "latest.riftreport.json" if run == "latest" else Path(run)
        return json.loads(path.read_text(encoding="utf-8"))

    def list_reports(self, limit: int = 20) -> dict[str, Any]:
        if limit <= 0:
            raise ValueError("limit must be positive")
        reports_dir = Path.cwd() / ".rift" / "reports"
    ...
    def _candidate_backend_decision(
        ...
        *,
        ...
        ranked = self.backend_adapters.rank(
            ...
            hardware=hardware,
            workload=workload,
            search_root=Path.cwd() / ".rift" / "backends",
        ...
        if offloaded and model_bytes > 0 and measured_limits:
            ...
            return {
                ...
                "claim_boundary": (
                    ...
                    "can reduce observed decode speed; run `rift recommend --verify` for an end-to-end result."
                ),
            }
        return {
            ...
            "claim_boundary": (
                ...
                "or measured offload I/O evidence. Use `rift recommend --verify`."
            ),
        }
    ...
    def _recommendation_user_answer(self, best: dict[str, Any]) -> dict[str, Any]:
        ...
        if absolute.get("backend") != "spoolstream_native_survival":
            tradeoffs.append(
